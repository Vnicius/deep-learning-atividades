...
def __compute_meshgrid(x, y):
    x_min, x_max, y_min, y_max = x[:, 0].min(
    ), x[:, 0].max(), x[:, 1].min(), x[:, 1].max()
...
def plot_data_and_predictions_3d_in_2d(x, y, is_binary, nn=None, threshold=0.0, figsize=(12, 6), s=15, cmap=plt.cm.viridis):
    ...
    if nn is not None:
        ...
        y_mesh = np.where(y_mesh <= threshold, 0, 1) if is_binary else np.argmax(
            __softmax(y_mesh), axis=1)

        plt.scatter(x[:, 0], x[:, 1], c=list(
            np.array(y).ravel()), s=s, cmap=cmap)
